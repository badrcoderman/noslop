...
assert.match(core, /const CARRIER_SLOTS = 4500000/);
assert.match(core, /const DRAIN_COUNT = 128/);
assert.match(core, /COMPOSE_DELAY_MS = CAPTURE_DELAY_MS \+ 500/);
assert.match(core, /capturedString = null;/);
assert.match(runtime, /PRIMITIVE-HISTORY-PURGE-PRE/);
assert.match(runtime, /history\.replaceState\(null, ""\)/);
assert.match(runtime, /settled-ms=750/);
