...
// noslop is intentionally exact-firmware only. The source snapshot has
// neighboring profiles, but reusing one here would turn a candidate into a
// false 13.60 claim.
const supportedFirmwares = ["13.60"];
...
if (!supportedFirmwares.includes(fw_str)) {
    if (typeof window.noslopMark === "function")
        window.noslopMark("FIRMWARE-REJECTED", fw_str || "not-a-ps5");
